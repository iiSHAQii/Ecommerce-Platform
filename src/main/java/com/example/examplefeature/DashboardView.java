...
import com.vaadin.flow.component.charts.model.style.SolidColor; // IMPORT ADDED
...
public class DashboardView extends VerticalLayout {
    ...
    // --- HELPER: APPLY DARK MODE SUPPORT ---
    private void applyTheme(Configuration conf) {
        // 1. Make Background Transparent
        // RGBA: 0,0,0,0 = Fully Transparent
        conf.getChart().setBackgroundColor(new SolidColor(0, 0, 0, 0));

        // 2. Set Text Colors to use Lumo CSS Variables (Must be wrapped in SolidColor)
        Style titleStyle = new Style();
        titleStyle.setColor(new SolidColor("var(--lumo-header-text-color)"));
        conf.getTitle().setStyle(titleStyle);

        Style legendStyle = new Style();
        legendStyle.setColor(new SolidColor("var(--lumo-body-text-color)"));
        conf.getLegend().setItemStyle(legendStyle);

        // 3. Axis Labels
        if (conf.getxAxis() != null) {
            conf.getxAxis().getLabels().getStyle().setColor(new SolidColor("var(--lumo-secondary-text-color)"));
            conf.getxAxis().getTitle().getStyle().setColor(new SolidColor("var(--lumo-secondary-text-color)"));
        }
        if (conf.getyAxis() != null) {
            conf.getyAxis().getLabels().getStyle().setColor(new SolidColor("var(--lumo-secondary-text-color)"));
            conf.getyAxis().getTitle().getStyle().setColor(new SolidColor("var(--lumo-secondary-text-color)"));
        }
    }
    ...
    private Component createPieChart(String title, List<Object[]> data) {
        ...
        plotOptions.setDataLabels(new DataLabels());
        plotOptions.getDataLabels().setColor(new SolidColor("var(--lumo-body-text-color)"));
